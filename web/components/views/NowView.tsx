...
export default async function NowView({ searchParams }: { searchParams: { coin?: string; all?: string } }) {
  ...
  if (error || !data) return <><h2 className="viewtitle">Signals</h2><SetupError error={error ?? "unknown"} /></>;

  return (
    <>
      <h2 className="viewtitle">Real-time signals</h2>
      ...
      <div className="tabs">
        <a href={`/signals?tab=now${onlyEvents ? "" : "&all=1"}`} className="on">{onlyEvents ? "Sudden events only" : "Including regular reads"}</a>
        <a href={`/signals?tab=now${onlyEvents ? "&all=1" : ""}`}>{onlyEvents ? "Show regular model reads too" : "Show sudden events only"}</a>
      </div>
